...
import { falRequestIdSchema, i2iInputSchema } from '$lib/server/ai-input';
...
export const POST = async (event: RequestEvent) => {
    ...
    const parsed = i2iInputSchema.safeParse(body);
    if (!parsed.success) {
        return json(
            { message: parsed.error.issues[0]?.message ?? 'Invalid image editing body.' },
            { status: 400 }
        );
    }
