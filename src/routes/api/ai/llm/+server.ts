...
/**
 * LLM 요청 입력 타입
 * - systemPrompt: AI의 동작 방식을 정의하는 시스템 프롬프트
 * - userPrompt: 사용자의 실제 요청
 */
interface LLMInput {
	systemPrompt?: string;
	userPrompt: string;
...
export const POST = async (event: RequestEvent) => {
	...
	// 2. JSON 바디 파싱
	const body = await readJson<LLMInput>(event);
	if (body instanceof Response) return body;

	// 3. userPrompt 필수 검증
	if (!body.userPrompt || typeof body.userPrompt !== 'string') {
		return json({ message: 'userPrompt is required and must be a string.' }, { status: 400 });
	}

	// 4. 메시지 배열 구성 (비즈니스 로직)
	const messages = buildMessages(body.systemPrompt, body.userPrompt);

	// 5. 스트리밍 요청 처리
	if (body.stream) {
		return handleStreamingRequest(messages, body);
	}

	// 6. 일반 요청 처리
	return handleCompletionRequest(messages, body);
};
